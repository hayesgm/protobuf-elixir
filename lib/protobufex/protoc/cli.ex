defmodule Protobufex.Protoc.CLI do
  def main(_) do
    ...
    IO.binwrite(Protobufex.Encoder.encode(response))
  end

  def run(bin) do
    Protobufex.Extensions.init()
  ...
  defp do_run(bin, run_extensions) do
    request = Protobufex.Decoder.decode(bin, Google.Protobuf.Compiler.CodeGeneratorRequest, run_extensions)
    # debug
    # raise inspect(request, limit: :infinity)
    ctx = %Protobufex.Protoc.Context{}
    ...
    if not run_extensions do
      for desc <- request.proto_file do
        content = Protobufex.Protoc.Generator.generate(ctx, desc).content
      ...
      files = request.proto_file
        ...
        |> Enum.map(fn(desc) -> Protobufex.Protoc.Generator.generate(ctx, desc) end)
