defmodule Protobufex.Decoder do
  import Bitwise, only: [bsl: 2, bsr: 2, band: 2]
  @mask64 bsl(1, 64) - 1

  alias Protobufex.{DecodeError, MessageProps, FieldProps}
        ...
        for extension_item <- extension do
          case extension_item do
            %Google.Protobuf.FieldDescriptorProto{extendee: extendee} when not is_nil(extendee) ->
              ...
              Protobufex.Extensions.add_extension_prop(extendee_module, {field_name, extension_item.number, [type: extender_module]})
          end
        end
      _ -> nil
    end
  ...
  defp get_module(str) do
    package_name = Protobufex.Protoc.Generator.Util.trans_type_name(str, %Protobufex.Protoc.Context{package: ""})
  ...
  defp put_field(:packed, msg, prop, field_key, val, _run_extensions) do
    vals = decode_packed(prop.type, Protobufex.Encoder.wire_type(prop.type), val)
