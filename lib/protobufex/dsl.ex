defmodule Protobufex.DSL do
  ...
  defmacro __before_compile__(env) do
    ...
    quote do
      ...
      def __extension_message_props__ do
        Protobufex.Extensions.get_extension_props(__MODULE__)
      end
  ...
  defp cal_label(%{required?: true}, :proto3) do
    raise Protobufex.InvalidError, message: "required can't be used in proto3"
  end
  ...
  defp cal_type(%{enum?: true, type: type} = props) do
    Map.merge(props, %{type: :enum, enum_type: type, wire_type: Protobufex.Encoder.wire_type(:enum)})
  end
  defp cal_type(%{type: type} = props) do
    Map.merge(props, %{type: type, wire_type: Protobufex.Encoder.wire_type(type)})
  end
  defp cal_type(props), do: props

  defp cal_default(%{default: default}, :proto3) when not is_nil(default) do
    raise Protobufex.InvalidError, message: "default can't be used in proto3"
  end
  ...
  def generate_default_fields(syntax, msg_props) do
    ...
    |> Enum.reduce(%{}, fn(props, acc) ->
         Map.put(acc, props.name_atom, Protobufex.Builder.field_default(syntax, props))
       end)
  end
