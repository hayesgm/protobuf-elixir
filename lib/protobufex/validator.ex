defmodule Protobufex.Validator do

  def validate!(struct) do
    case valid?(struct) do
      true -> true
      {:invalid, msg} -> raise(Protobufex.InvalidError, message: msg)
    end
  end

  def valid?(%{__struct__: mod} = struct) do
    msg_props = mod.__message_props__
    valid = if Protobufex.MessageProps.has_oneof?(msg_props), do: oneof_valid?(struct, msg_props), else: true
